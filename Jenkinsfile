pipeline {
    ...
    environment {
        repoApp= "https://github.com/davidbermudez/maven-project-jenkins"
        rama = "master"

        dateTime = sh(returnStdout: true, script: 'date +%Y%m%d-%H%M').trim()
        imagen = "dbermudez1972/testing-jenkins:${dateTime}"
    }
                ...
                dir(path: 'contenedor') {
                    withCredentials(bindings: [usernamePassword(credentialsId: 'github_credential',
                        ...
                        sh '''
                            ...
                            git add .
                            git config --global user.email \'davidbermudez@jerez.es\'
                            git config --global user.name  \'davidbermudez\'
